        ...
        if self.enable_interference:
            ...
            for i, frame in enumerate(interferer_per_frame):
                ...
                for j, radar in enumerate(affected_radars):
                    ...
                    # min_interferer_dist = np.sum(distance_per_radar[j])
                    ...
                    # if distance_per_radar[j].size > 1:
                    #     total_int_power = np.sum(interference(distance_per_radar[j]))
                    #     print(
                    #         f"Total int {total_int_power} single int {int_power} signal {sig_power[0]}"
                    #     )
